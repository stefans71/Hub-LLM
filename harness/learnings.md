...
**Key Learning**: When two subsystems connect independently (terminal via WebSocket, chat via backend state check), they need a bridge callback. The terminal connects first but chat didn't know. Threading a callback through the component tree solves this without adding global state.
...
**Key Learning**: When conditional styling overrides a mode selector, the fix is to make the override conditional on BOTH the context AND the mode, not just the context.
...
### Session 147 - 2026-02-09 EST
**Task**: FEAT-53 — Improve /generate-prp intake flow
**What**: Rewrote TEMPLATE_GENERATE_PRP with 6 additions: (1) Navigation commands ("change answer [N]", "show my answers", "start over") available throughout all phases. (2) User Profile rewrite as living document — continuous updates with Q&A Behavior tracking (revisions, expansions, restarts, free-text usage, projects completed), read-before-starting for returning users, project scope awareness calibrating Q&A depth. (3) Running answer summary maintained through all phases. (4) Review gate (new Phase 3) — interactive per-answer review with Keep/Expand/Change + final confirmation. (5) Post-PRP approval gate (new Phase 5) — approve, change specific answer (loops back to Q&A then regenerates), or start over. (6) Speech-to-text tip for first-time users. Phase numbering shifted: old Phase 3 → Phase 4 (generation), new Phase 3 = review gate, new Phase 5 = approval gate. Free-text option added to every question. projects.py 1416→1526 lines.
**Key Learning**: Prompt templates that instruct Claude to behave interactively need explicit phase gating — "Do NOT proceed until user selects X" — because Claude's default behavior is to auto-continue. Separating review gate (Phase 3) from generation (Phase 4) from approval (Phase 5) creates clear stopping points. The User Profile as a living document means every interaction is behavioral data that accumulates across projects.

### Session 148 - 2026-02-09 EST
**Task**: FEAT-56 — code-researcher agent template
...
### Session 153 - 2026-02-10 EST
**Task**: FEAT-62 — Welcome experience V3: dynamic Getting Started page + slim terminal message
**What**: (Part A) New TEMPLATE_GETTING_STARTED_HTML in projects.py — self-contained HTML page with inline CSS (dark theme #0f1419), brand colors, project-specific placeholders ({{projectName}}, {{slug}}, {{appDir}}). New GET /{project_id}/getting-started endpoint renders template dynamically (no VPS file). (Part B) Slimmed TEMPLATE_DIRECTOR_WELCOME text: removed step 3 (/generate-prp), removed "You will be managing..." line, added orange "See Getting Started →" pointer, "STARTING:" heading, single orange Note line for copy/paste. 2 steps only. (Part C) Workspace.jsx: welcomeUrl now uses /api/projects/{id}/getting-started when project exists, fallback /docs/welcome.html. ClaudeCodeTerminalChat.jsx: deleted "Type claude..." writeln, deleted entire showCommandHint help bubble + state. projects.py 1743→1845 lines.
**Key Learning**: Dynamic template rendering (endpoint returns HTML) > scaffolded files for content that needs to stay current. Template change → deploy → ALL projects updated. No VPS file management or orphan cleanup needed. For self-contained iframe HTML, inline all CSS — never depend on external stylesheets from the parent app.

### Session 154 - 2026-02-10 EST
**Task**: FEAT-63 — Fix Getting Started rendering: full-bleed, scroll, nav sidebar, Docs button
**What**: (1) PreviewPanel.jsx: deleted `isInternalDocs`, changed `isFullBleed = deviceMode === 'fit'` — ALL content renders full-bleed in Fit mode, not just /docs/ URLs. (2) Fixed scrolling: `overflow: isFullBleed ? 'auto' : 'hidden'` on inner frame container. (3) Docs button: navigates to `previewUrl` (project Getting Started) instead of hardcoded /docs/index.html. iframe default src also uses previewUrl. (4) Added inline nav sidebar to TEMPLATE_GETTING_STARTED_HTML: 220px sidebar with brand header, tagline, 6 nav links (Home, Getting Started active, Workspace Guide, Git & GitHub, What's New, Harness System). Responsive: sidebar flows above content on narrow viewports via @media (max-width: 600px). projects.py 1845→1881 lines.
**Key Learning**: For iframe content rendered via dynamic endpoints (not /docs/ prefix), `isInternalDocs` URL check breaks. Simpler: `isFullBleed = deviceMode === 'fit'` — all content goes edge-to-edge in Fit mode regardless of URL origin.

### Session 155 - 2026-02-10 EST
**Task**: FEAT-64 — Migrate all docs to dynamic backend templates
**What**: Created backend/routers/docs.py (778 lines) with 5 endpoints: /home, /workspace-guide, /git-github, /whats-new, /harness. Each returns self-contained HTMLResponse with inline CSS and baked-in nav sidebar. Shared CSS constant (DOCS_CSS), nav builder function (_nav_sidebar), page wrapper (_docs_page). Home page includes inline search JS. Registered router in main.py with prefix=/api/docs. Updated TEMPLATE_GETTING_STARTED_HTML nav links from /docs/*.html to /api/docs/*. Updated Workspace.jsx fallback and PreviewPanel.jsx defaults to /api/docs/home. Deleted frontend/public/docs/ directory (8 files: 5 HTML, styles.css, nav.js, welcome.html).
**Key Learning**: For multi-page docs in an iframe, a shared CSS constant + nav builder function + page wrapper is cleaner than duplicating full HTML. Each page is a content constant; the wrapper adds CSS, nav sidebar (with correct active class), and structure. Keep SVGs out of nav links (use text only) to reduce template bloat — SVG icons add ~2KB per page for minimal UX benefit.
